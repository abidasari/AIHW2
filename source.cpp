...
{
public:
	int whoStarts, cutoffDepth, n_pits, arrSize;//pits keeps count of the number of pits each player has(other than his mancala)
...
{
	//what move to make?
	//all the possible moves... of course???
	// int *root = new int[arrSize];
	// boardBackup(b , root);
	int c = depth;
	...
	if(c < cutoffDepth)
	{
		...
		// for(int t = 0 ; t < arrSize ; t++)//no more possible moves to make so, we reset the board, decrement the counter and return the move
		// 	board1[t] = b[t];
		// c--;
