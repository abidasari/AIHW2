...
int _inf = std::numeric_limits<int>::min();
...
public:
	...
	move Max_play(int * b , int count, int player, std::string s);
	move Min_play(int * b , int count, int player, std::string s);
	void minimaxUtil();
...
void mancala::minimaxUtil()
{
	...
	move v = Max_play(board , 0, whoStarts, "root,0,-Infinity\n");
	o_file.close();
}
...
move mancala::Max_play(int * b, int depth, int player, std::string parentMove)
{
	int c = depth;
			...
			for(int i = 0 ; i < n_pits ; i++ )
			{
				if(board1[i] != 0)
				{
					...
					if(is_re_move)
					{
						...
						move l = Max_play(board1 , c-1, player, S);
						for(int t = 0 ; t < arrSize ; t++)
							board1[t] = b[t];
						c--;
					}
...
move mancala::Min_play(int* b , int count, int player, std::string parentMove)
{
	o_file << parentMove;// << "line above printed in min 1\n";
	int board2[22] = { };
	bool flip = true;
	std::string Q;
	for(int t = 0 ; t < arrSize ; t++)
			...
			for(int i = arrSize-2 ; i > n_pits ; i-- )
			{
				if(board2[i] != 0)
				{
					...
					if(is_re_move)
					{
						...
						move o = Min_play(board2, k-1, player, Q);
						for(int t = 0 ; t < arrSize ; t++)
							board2[t] = b[t];
						k--;
					...
					o_file << parentMove;// << "line above printed in min 2.1\n";
				}
